...
public class HoverSizeChanger : MonoBehaviour {
	...
	void ObjectLoader()//loads all neccessay objects
	{
		...
		if (Boarders.Count == 0) // 畫線
		{
			...
			Boarders.Add(new CustomRectTransformPos(transform.Find("LowerPart").Find("Boarder3").GetComponent<RectTransform>(), new Vector2(2007, 100)));
		}
		...
		if (LowerPart == null)
		{
			...
			LowerPartPos = new Vector2(2007,100);
		}
