...
public class LoadVictoryStory : MonoBehaviour {
    ...
    void Start () {
        ...
        dialogueActive = true;
        currentLine = 1 ;
        dText.text = ReadFile("Assets\\Text\\Victory_story.txt", currentLine);
	...
	void Update () {
        ...
        {
            dText.text = ReadFile("Assets\\Text\\Victory_story.txt", currentLine); // 目前只有找到利用絕對路徑來存取的方法，希望能改成相對路徑(同一資料夾讀取)
        }
    ...
    void OnGUI()
    {
        ...
        style.wordWrap = true ;
        GUI.Label(new Rect(Screen.width / 2 - 250, Screen.height / 2 - 150, 200, 200), "終章", style); // 使用GUI函數來固定text顯示位置
