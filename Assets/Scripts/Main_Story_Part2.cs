...
public class Main_Story_Part2 : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        dialogueActive = true;
        currentLine = 1;
        dText.text = ReadFile("Assets\\Text\\Main_story_part2.txt", currentLine);
        text = dText.text;
    ...
    void Update()
    {
        ...
        if (canvasGroup.alpha == 1)
        {
            ...
            if (currentLine == 1)
            {
                StartCoroutine(TypeText()); // 在main story中要把第一句的打字機移過來這裡才能避免根本還沒trigger到就開始劇情
                currentLine++;
            }
            ...
            {
                dText.text = ReadFile("Assets\\Text\\Main_story_part2.txt", currentLine); // 目前只有找到利用絕對路徑來存取的方法，希望能改成相對路徑(同一資料夾讀取)
            }
