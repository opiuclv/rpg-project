...
public class Red2SlimeControler : MonoBehaviour
{
                ...
                case MonsterState.ATTACK:
                    ...
                    if (Time.time - lastAttackTime > attackCDTime)
                    {
                        ...
                        Vector3[] bulletPos = { transform.position,
                                    new Vector3(transform.position.x + aimsDir.y, transform.position.y - aimsDir.x, transform.position.z),
                                    new Vector3(transform.position.x - aimsDir.y, transform.position.y + aimsDir.x, transform.position.z) };

                        for (int i = 0; i < bulletPos.Length; i++)
                        {
                            var clone = (GameObject)Instantiate(attackObject, bulletPos[i], Quaternion.Euler(Vector3.zero));
